...
export class MessageGatewayv1 implements OnGatewayConnection, OnGatewayDisconnect {
  @WebSocketServer()
  server: Server;
  ...
  async handleMessage(socket: Socket, payload: { chatId: string, to: string, message: string }) {
    ...
    const { username: from, role } = socket["userCred"] as { username: string, role: string };
    console.log(payload);
    const storedMessage = await this.messageService.createMessage(chatId, from, to, message);
